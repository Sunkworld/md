program main
   ...
   real(8),dimension(nsample,0:ncor) :: Ek_acf, Ep_acf, Et_acf
   real(8),dimension(0:ncor) :: Ek_cor_avg, Ep_cor_avg, Et_cor_avg
   real(8),dimension(0:ncor) :: Ek_cor_std, Ep_cor_std, Et_cor_std
...
!open(11, file="traj_1")
	 ...
	 Et_avg(j) = Et_avg(j) + Ek(i) + Ep(i)
      ...
      call acf(Ek_acf(j,:), Ek, n, ncor)
      call acf(Ep_acf(j,:), Ep, n, ncor)
      call acf(Et_acf(j,:), Et, n, ncor)
      ...
      Ek_acf(j,:) = Ek_acf(j,:) / Ek2_avg
      Ep_acf(j,:) = Ep_acf(j,:) / Ep2_avg
      Et_acf(j,:) = Et_acf(j,:) / Et2_avg

      Ek_cor_avg = (Ek_cor_avg*dble(j-1)+Ek_acf(j,:))/dble(j)
      Ep_cor_avg = (Ep_cor_avg*dble(j-1)+Ep_acf(j,:))/dble(j)
      Et_cor_avg = (Et_cor_avg*dble(j-1)+Et_acf(j,:))/dble(j)
  end do
   ...
   do i = 1, ncor
    ...
    do j = 1, nsample
      Ek_acf(j,i) = Ek_acf(j,i-1)+Ek_acf(j,i)
      Ep_acf(j,i) = Ep_acf(j,i-1)+Ep_acf(j,i)
      Et_acf(j,i) = Et_acf(j,i-1)+Et_acf(j,i)
    enddo
Ek_cor_std(i)=sqrt(sum((Ek_acf(:,i)-Ek_cor_avg(i))**2)/(nsample-1)/nsample)
Ep_cor_std(i)=sqrt(sum((Ep_acf(:,i)-Ep_cor_avg(i))**2)/(nsample-1)/nsample)
Et_cor_std(i)=sqrt(sum((Et_acf(:,i)-Et_cor_avg(i))**2)/(nsample-1)/nsample)
      write(12, '(I8,F16.8,F16.8,F16.8,F16.8,F16.8,F16.8)') i, Ek_cor_avg(i)/Ek_cor_avg(0), Ep_cor_avg(i)/Ep_cor_avg(0),&
& Et_cor_avg(i)/Et_cor_avg(0), Ek_cor_std(i), Ep_cor_std(i), Et_cor_std(i)
   end do
