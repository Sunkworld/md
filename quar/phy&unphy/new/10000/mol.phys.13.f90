...
!  integer, parameter :: tsstep=1d7/h
  ...
  integer,parameter :: ndt=15000!needs to be modified
...
!    h = 0.02d0
    gamma2 = 10000d0                                                                      !needs to be modified
    a = exp(-gamma2*h)
    lambda = (2-2*a)/(a+1)
    b = (a+1)/2
    gamma = lambda/h
!    ndt = ceiling(tottime/tt0/h)
    ...
    allocate(cor_ep(0:ndt-1,sample,2),cor_ek(0:ndt-1,sample,2),corep(sample),corek(sample))
    cor_ep(:,:,:)=0d0
    cor_ek(:,:,:)=0d0
!    h = 0.1d0 * dt
...
!       open(12, file=trim(adjustl(aa))//'-x.txt')
    ...
    !   write(*,*) 'sample=', j
      ...
      !      write(*,*) i-1
            ep0(nt0)=eptmp
            ek0(nt0)=ektmp
            cor_ep(0,j,1) = cor_ep(0,j,1)+eptmp**2
            cor_ep(0,j,2) = cor_ep(0,j,2)+eptmp*2
            cor_ek(0,j,1) = cor_ek(0,j,1)+ektmp**2
            cor_ek(0,j,2) = cor_ek(0,j,2)+ektmp*2
    !        write(*,*) cor_ep(0,j)
  !          write(*,*) nt0,eptmp
          else
            n=mod(i-1,ndt)
            cor_ep(n,j,1)=cor_ep(n,j,1)+eptmp*ep0(nt0)
            cor_ep(n,j,2)=cor_ep(n,j,2)+eptmp+ep0(nt0)
            cor_ek(n,j,1)=cor_ek(n,j,1)+ektmp*ek0(nt0)
            cor_ek(n,j,2)=cor_ek(n,j,2)+ektmp+ek0(nt0)
  ...
  !           tmp=(qn+bound)/width
    ...
    !   close(11)
       ...
       cor_ep(:,j,1)=(cor_ep(:,j,1)/tt0-cor_ep(:,j,2)/tt0*ep(j)+ep(j)**2)/(ep2(j)-ep(j)**2)
       cor_ek(:,j,1)=(cor_ek(:,j,1)/tt0-cor_ep(:,j,2)/tt0*ek(j)+ek(j)**2)/(ek2(j)-ek(j)**2)
  !     write(100,'(I8,F16.8,F16.8,F16.8,F16.8)') j, ep(j), ek(j), ep2(j), ek2(j)
    write(*,*) j!cor_ep(0,j,2)
...
!    open(11,file="a="//trim(adjustl(bb))//"_dt="//trim(adjustl(cc))//"_cor_energy.dat")
    do i=1,sample
        corep(i)=sum(cor_ep(:,i,1))
        corek(i)=sum(cor_ek(:,i,1))

!  write(11,"(I5,3(2x,F20.12))") i, i*h, corep(i), corek(i)
     end do
     do i=0,ndt-1
        write(233,'(I8,F16.8)') i,sum(cor_ep(i,:,1))/sample
        enddo
  !  close(11)
