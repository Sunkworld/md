...
for i in opt_pot opt_har omega twiomega
do
  cd $i
  ...
  do
    ...
    cp ${HOME_DIR}/exact.csv .
    paste tmp0_k.csv tmp1_k.csv tmp2_k.csv tmp3_k.csv tmp4_k.csv tmp5_k.csv tmp6_k.csv tmp7_k.csv tmp8_k.csv tmp9_k.csv tmp10_k.csv tmp11_k.csv exact.csv>$i-$j-tmp-k.csv
    paste tmp0_p.csv tmp1_p.csv tmp2_p.csv tmp3_p.csv tmp4_p.csv tmp5_p.csv tmp6_p.csv tmp7_p.csv tmp8_p.csv tmp9_p.csv tmp10_p.csv tmp11_p.csv exact.csv>$i-$j-tmp-p.csv
    if [ "$j" = "physical" ]
    then
      sed -i '' '1i\
      ...
      sed -i '' '1i\
        dt,middle,middle-error,end,end-error,beginning,beginning-error,side,side-error,PV-middle,PV-middle-error,PV-end,PV-end-error,PV-beginning,PV-beginning-error,PV-side,PV-side-error,dt,exact
      '  $i-$j-tmp-k.csv
      sed -i '' '1i\
        dt,middle,middle-error,end,end-error,beginning,beginning-error,side,side-error,PV-middle,PV-middle-error,PV-end,PV-end-error,PV-beginning,PV-beginning-error,PV-side,PV-side-error,dt,exact
      '  $i-$j-tmp-p.csv
